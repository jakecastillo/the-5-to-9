...
a cross-tool AI night-shift crew. This package is a pure-Node command line plus a
live interactive Ink TUI over the existing `driver/` engine.
...
| `the-5-to-9 dashboard` | One-shot dashboard view; `--watch` launches the live interactive Ink TUI. |
...
Running `the-5-to-9` with no subcommand launches the live interactive Ink TUI
directly (the same view as `dashboard --watch`).
