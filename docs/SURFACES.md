...
[ARCHITECTURE.md](ARCHITECTURE.md). The core is tool-agnostic; only the *plugin wiring*
...
in a *subdirectory*, not at the marketplace root. Tracked by **`ap8`**. Until then the
skill *bodies* are fully portable: Codex reads them directly.
